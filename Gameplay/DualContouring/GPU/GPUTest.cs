...
namespace MHGameWork.TheWizards.DualContouring.GPU
{
    ...
    public class GPUTest
    {
        ...
        private static void saveTest3DTexture(DX11Game game, int size, string name, byte[] data)
        {
            ...
            Texture3D.SaveTextureToFile(game.Device.ImmediateContext, texture3D.Resource, ImageFileFormat.Dds,
                TWDir.Test.CreateSubdirectory("DualContouring.GPU").CreateFile(name + ".dds").FullName);
            texture3D.SaveToImageSlices(game,TWDir.Test.CreateSubdirectory("DualContouring.GPU").CreateSubdirectory(name));
        }
        ...
        public void TestCalculateDensityGridSigns()
        {
            ...
            var density = "20-v.y";
            var target = new GPUHermiteCalculator();
            var tex3d = target.CalculateGridPoints(size, offset, scaling, density);
    ...
    {
        public GPUTexture3D CalculateGridPoints(int size, Vector3 offset, Vector3 scaling, string density)
        {
