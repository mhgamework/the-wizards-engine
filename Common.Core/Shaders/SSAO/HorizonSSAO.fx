...
//PostProc_VSOut FullScreenQuadVS( VS_IN input )
//{
...
float3 fetch_eye_pos(float2 uv)
{
	...
	float depth = tLinDepth.SampleLevel(samNearest, float3(uv, 0).xy, 0);
	...
	//return float3(-position.z/100,0,0);

	float eye_z = (-position.z- 0.1f)/(400-0.1f);
	//return float3(eye_z,0,0);
	
	return uv_to_eye(uv,eye_z);
	return position.xyz;

    float z = tLinDepth.SampleLevel(samNearest, float3(uv, 0).xy, 0); //MHGW Added .xy
	return float3(z,0,0);
...
float biased_tangent(float3 T)
{
	...
	//return tan(clamp(phi,0,M_PI*0.5 -0.1));
//return tangent(T);
//return tan(M_PI*0.5);
	//return min(phi,M_PI*0.5)/2;

    return tan(min(phi, M_PI*0.5-0.0001)); //MHGW: Added the 0.0001
}
...
float4 HORIZON_BASED_AO_PS( uniform bool useNormal, uniform int qualityMode, PostProc_VSOut IN ) : SV_TARGET
{
	...
	if (false) {
		...
		float3 N = normalize(DecompressNormal( tNormal.SampleLevel(samNearest, IN.texUV,0).rgb )) ;
	...
	//float3 tempN = float3(0,0,0);
    //tangentPlane = float4(tempN, dot(P, tempN));
