...
{
    public class TestShadowsScattering : IUnitTestGame
    {
        ...
        public TestShadowsScattering()
        {
            ...
            RendererService = new MHGameWork.TheWizards.ServerClient.TWClient.RendererService( this.xnaGame, database );
            database.AddService( RendererService );


            //Sky.SkyRenderer skyRenderer = new Sky.SkyRenderer( database );
            CascadedShadowMaps.CSMRendererService csmRendererService = new CascadedShadowMaps.CSMRendererService( xnaGame, database );
            database.AddService( csmRendererService );
            Sky.SkyCSMRenderer skyCSMRenderer = new MHGameWork.TheWizards.ServerClient.Sky.SkyCSMRenderer( database );
            RendererService.AddIRenderer( csmRendererService );



            // Load all plugins in assembly
            List<IPlugin002> plugins = Editor.PluginLoader.GetPlugins<IPlugin002>( AppDomain.CurrentDomain.GetAssemblies() );
