...
Namespace Terrain.Client
    Public Class CornerData
        ...
        End Sub
        Public Sub New(ByVal nParentCD As CornerData, ByVal nParentSquare As Square, ByVal nDir As Byte)
            Me.New(nParentCD, nParentSquare, CType(nDir, ChildDir))
        End Sub

        Public Sub New(ByVal nParentCD As CornerData, ByVal nParentSquare As Square, ByVal nDir As ChildDir)
            'Me.New()
            ...
            'Me.ChildIndex = nDir
            'Me.Square = nParentSquare.Child(nDir)
            ...
            '        Me.CornerHeight(CornerDir.NorthWest) = nParentSquare.Height(HeightDir.North)
            '        Me.CornerHeight(CornerDir.SouthWest) = nParentSquare.Height(HeightDir.Center)
            '        Me.CornerHeight(CornerDir.SouthEast) = nParentSquare.Height(HeightDir.East)

            '        Center += HalfSize

            '    Case ChildDir.NorthWest
            '        Me.CornerHeight(CornerDir.NorthEast) = nParentSquare.Height(HeightDir.North)
            '        Me.CornerHeight(CornerDir.NorthWest) = nParentCD.CornerHeight(CornerDir.NorthWest)
            '        Me.CornerHeight(CornerDir.SouthWest) = nParentSquare.Height(HeightDir.West)
            '        Me.CornerHeight(CornerDir.SouthEast) = nParentSquare.Height(HeightDir.Center)

            '        Center += New Vector2(-HalfSize.X, HalfSize.Y)

            '    Case ChildDir.SouthWest
            '        Me.CornerHeight(CornerDir.NorthEast) = nParentSquare.Height(HeightDir.Center)
            '        Me.CornerHeight(CornerDir.NorthWest) = nParentSquare.Height(HeightDir.West)
            '        Me.CornerHeight(CornerDir.SouthWest) = nParentCD.CornerHeight(CornerDir.SouthWest)
            '        Me.CornerHeight(CornerDir.SouthEast) = nParentSquare.Height(HeightDir.South)

            '        Center -= HalfSize

            '    Case ChildDir.SouthEast
            '        Me.CornerHeight(CornerDir.NorthEast) = nParentSquare.Height(HeightDir.East)
            '        Me.CornerHeight(CornerDir.NorthWest) = nParentSquare.Height(HeightDir.Center)
            '        Me.CornerHeight(CornerDir.SouthWest) = nParentSquare.Height(HeightDir.South)
