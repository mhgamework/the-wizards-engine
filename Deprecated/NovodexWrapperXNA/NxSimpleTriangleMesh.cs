...
namespace NovodexWrapper
{
	...
	public class NxSimpleTriangleMesh
	{
		...
		protected Vector3[] internalPointArray=null;	//this in case the pointArray that the user passed in is deleted before the object is created
		protected int[] internalTriangleArray=null;		//this in case the triangleArray that the user passed in is deleted before the object is created
...
FlagSixteenBitIndices=false;
		...
		{
			get{return NovodexUtil.areBitsSet(flags,(uint)NxMeshFlags.NX_MF_FLIPNORMALS);}
			set{flags=NovodexUtil.setBits(flags,(uint)NxMeshFlags.NX_MF_FLIPNORMALS,value);}
		}
		
		public bool FlagSixteenBitIndices
		{
			get{return NovodexUtil.areBitsSet(flags,(uint)NxMeshFlags.NX_MF_16_BIT_INDICES);}
			set{flags=NovodexUtil.setBits(flags,(uint)NxMeshFlags.NX_MF_16_BIT_INDICES,value);}
		}

		public bool FlagHardwareMesh
		{
			get{return NovodexUtil.areBitsSet(flags,(uint)NxMeshFlags.NX_MF_HARDWARE_MESH);}
			set{flags=NovodexUtil.setBits(flags,(uint)NxMeshFlags.NX_MF_HARDWARE_MESH,value);}
		}
	}
}
