...
{
    public class SpectaterCamera:GameComponent
    {
        Game1 game;
        ...
        public SpectaterCamera(Game1 nGame ):base(nGame)
        {
            game = nGame;
        ...
        public void Process(GameTime time)
        {
            ...
            if ( keyboard.IsKeyDown( Keys.S ) ) { vSnelheid += Vector3.Forward; }
            if ( keyboard.IsKeyDown( Keys.Z ) ) { vSnelheid += Vector3.Backward; }
            if ( keyboard.IsKeyDown( Keys.Q ) ) { vSnelheid += Vector3.Right; }
            if ( keyboard.IsKeyDown( Keys.D ) ) { vSnelheid += Vector3.Left; }
            if ( keyboard.IsKeyDown( Keys.Space ) ) { vSnelheid += Vector3.Up; }
            if ( keyboard.IsKeyDown( Keys.LeftControl ) ) { vSnelheid += Vector3.Down; }
            ...
            {
                AngleHorizontal += MathHelper.ToRadians( mouse.X-previousMouseX );
            ...
            if ( mouse.Y != previousMouseY )
            {
                ...
                if ( MathHelper.ToRadians( mouse.Y-previousMouseY ) < MathHelper.PiOver2 ) { };
                AngleVertical = MathHelper.Clamp( AngleVertical - MathHelper.ToRadians( mouse.Y-previousMouseY ), -MathHelper.PiOver2, MathHelper.PiOver2 );
            ...
            //Me.Camera.Process()
            Positie += Snelheid * (float)time.ElapsedGameTime.TotalSeconds;
