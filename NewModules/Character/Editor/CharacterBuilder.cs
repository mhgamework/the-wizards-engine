...
namespace MHGameWork.TheWizards.Character.Editor
{
    public class CharacterBuilder
    {
        ...
        private Character skinnedModel;
        ...
        public Character CreateCharacter( IXNAGame game, ColladaModel _colladaModel )
        {
            Character ret;

            colladaModel = _colladaModel;
            skinnedModel = new Character( game );
            ret = skinnedModel;
        ...
        private void LoadBone( CharacterBone parentBone, ColladaBone colladaBone )
        {
            CharacterBone bone = new CharacterBone( skinnedModel.Game );
        ...
        public void LoadMeshes()
        {
            ...
            for ( int i = 0; i < colladaModel.Meshes.Count; i++ )
            {
                ...
                {
                    CharacterMesh mesh = new CharacterMesh( skinnedModel );
        ...
        private void LoadFromColladaSceneNode( ColladaSceneNodeBase node )
        {
            ...
            if ( node.Instance_Geometry != null )
            {
                ...
                for ( int iPart = 0; iPart < mesh.Parts.Count; iPart++ )
                {
                    ...
                    CharacterMesh skinnedMesh = new CharacterMesh( skinnedModel );
        ...
        private void LoadMesh( CharacterMesh mesh, ColladaMesh colladaMesh, ColladaMesh.PrimitiveList primitives, Matrix objectMatrix )
        {
        ...
        /// <returns></returns>
        private void LoadMeshMaterial( CharacterMesh mesh, ColladaMaterial mat )
        {
        ...
        public static Character LoadCharacter( IXNAGame game, Stream colladaStream )
        {
            ColladaModel colladaModel = ColladaModel.FromStream( colladaStream );

            CharacterBuilder builder = new CharacterBuilder();

            return builder.CreateCharacter( game, colladaModel );


        }
        public static Character LoadCharacter( IXNAGame game, ColladaModel model )
        {
            CharacterBuilder builder = new CharacterBuilder();

            return builder.CreateCharacter( game, model );
