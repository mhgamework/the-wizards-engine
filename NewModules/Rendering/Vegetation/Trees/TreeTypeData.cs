...
namespace MHGameWork.TheWizards.Rendering.Vegetation.Trees
{
    ...
    public class TreeTypeData
    {
        ...
        public void WriteToXML(Stream st)
        {
            ...
            for (int i = 0; i < Levels.Count; i++)
            {
                ...
                for (int l = 0; l < Levels[i].LeafType.Count; l++)
                {
                    ...
                    TreeGenerator.RangeSpreading.WriteToXML(ileafLevel.RelativePosition, leafNode.CreateChildNode("RelativePosition"));
                    TreeGenerator.RangeSpreading.WriteToXML(ileafLevel.AxialSplitPosition, leafNode.CreateChildNode("AxialSplit"));
                    TreeGenerator.RangeSpreading.WriteToXML(ileafLevel.DropAngle, leafNode.CreateChildNode("DropAngle"));
                    ...
                    leafNode.AddChildNode("Bump", null);
        ...
        {
            //var fac = new SimpleTextureFactory();
            AddTestRAMTextures(fac);
            return GetTestTreeType(fac);
        }
        public static TreeTypeData GetTestTreeType(ITextureFactory textureFactory)
        {
            Stream st =
                File.OpenRead(
                    @"C:\_MHData\1 - Projecten\The Wizards\_Source\NewModules\TreeGenerator\Files\TestTree.XML");
                //MHGameWork.TheWizards.Common.Core.EmbeddedFile.GetStream("NewModules.TreeGenerator.Files.TestTree.XML","TestTree.XML");
        ...
        /// <summary>
        /// TODO: move to tests, THIS IS A TEST METHOD
        /// </summary>
        /// <param name="texFact"></param>
