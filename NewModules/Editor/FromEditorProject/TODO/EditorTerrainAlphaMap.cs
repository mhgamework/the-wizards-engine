...
{
    public class EditorTerrainAlphaMap : IDisposable
    {
        protected int width = 0;
        protected int length = 0;
        protected byte[] data;

        public EditorTerrainAlphaMap( int nWidth, int nLength )
        {
        ...
        private bool Create( int width, int length )
        {
        ...
        {
            const string indexFormula = "i = z * width + x";

            node.AddAttributeInt( "Width", width );
            node.AddAttributeInt( "Length", length );
            node.AddAttribute( "IndexFormula", indexFormula );
            node.AddCData( Convert.ToBase64String( data ) );
        }

        public static EditorTerrainAlphaMap LoadFromXml( TWXmlNode node )
        {
            ...
            if ( "i = z * width + x".Equals( indexFormula ) == false ) throw new Exception( "Invalid data format!!" );


            byte[] data = Convert.FromBase64String( node.ReadCData() );

            EditorTerrainAlphaMap map = new EditorTerrainAlphaMap( width, length );
